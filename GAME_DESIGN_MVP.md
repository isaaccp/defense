...
3. **The Shared Focus Pool:** Instead of individual mana bars, the 2-hero party shares a single **Party Focus Pool**. 
   - **Active Generation:** Individual heroes do *not* have intrinsic passive focus or focus_regen attributes. Instead, they provide *bonus* focus generation to the shared pool based strictly on free actions or game states (avoiding "focus discounts" from spending focus on spells). For example, the Knight generates focus from raw unmitigated incoming damage; the Priest generates focus based on missing party HP; the Rogue generates focus via kills and attacking wounded enemies; the Wizard generates focus by remaining idle.
   - **Death Handling:** Because the Tower provides the baseline passive focus regeneration, a hero dying does not instantly shrink the party's Max Focus pool or cripple the baseline regen. The surviving hero simply loses the bonus focus generation the dead hero was providing.

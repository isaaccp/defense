...
- ~~**Meta Progression UI / `meta_xp` cleanup:** `meta_xp` has been ripped out of the state model and documentation, but `skill_tree_ui.gd` still tries to use it in `Mode.UNLOCK` (Meta Progression screen). We need to answer three design questions to finish the refactor:~~ (Completed 2026-06-19)

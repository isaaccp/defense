...
func test_sweeping_attack_one_enemy():
	...
	enemy_hurtbox = enemy.get_component_or_die(HurtboxComponent)
	...
	assert_signal_emit_count(enemy_hurtbox, "hit", 1)
...
func test_sweeping_attack_multiple_enemies():
	...
	var hurtbox_components = []
	for i in range(3):
		var enemy = TestUtils.make_barrel(10)
		level.enemies.add_child(enemy)
		enemy.position = character.position + offsets[i]
		hurtbox_components.append(enemy.get_component_or_die(HurtboxComponent))

	level.start()

	for i in range(3):
		watch_signals(hurtbox_components[i])

	# Wait for one second.
	await wait_seconds(1.0)

	for i in range(3):
		assert_signal_emit_count(hurtbox_components[i], "hit", 1)

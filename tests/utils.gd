...
static func count_action_triggered(test: GutTest, behavior: BehaviorComponent, action_id: ActionDef.Id):
	var count = 0
...
static func assert_last_action(test: GutTest, behavior: BehaviorComponent, expected: ActionDef.Id):
...
static func assert_last_action_not(test: GutTest, behavior: BehaviorComponent, expected: ActionDef.Id):
	var count = test.get_signal_emit_count(behavior, "behavior_updated")
	test.assert_ne(count, 0, "No actions emitted")
	var params = test.get_signal_parameters(behavior, "behavior_updated")
	var got = params[0]

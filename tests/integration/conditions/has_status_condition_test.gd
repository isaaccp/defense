...
const has_status_def = preload("res://skill_tree/conditions/has_status.tres")
...
func make_behavior(invert: bool) -> StoredBehavior:
	var condition_params = SkillParams.new()
	condition_params.set_placeholder_value(SkillParams.PlaceholderId.BOOL_VALUE, invert)
	condition_params.set_placeholder_value(SkillParams.PlaceholderId.STATUS, taunted_status)
	
	var condition = StoredParamSkill.new()
	condition.name = has_status_def.skill_name
...
func test_passes_when_has_status():
	...
	enemy_status.set_status(&"test", taunted_status, TauntedParams.make(character), 5.0)

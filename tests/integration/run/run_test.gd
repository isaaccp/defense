...
func before_each():
	...
	var milestone_manager = MilestoneManager.new(save_state, level_provider.milestone_library)
	run.initialize(RunSaveState.make([], level_provider, SkillTreeState.new(), save_state.unlocked_milestones, save_state.milestone_progress), ui_layer, milestone_manager)

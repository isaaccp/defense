...
- **How to run**: By default, the `.gutconfig.json` file is configured to *only* run the `tests/unit/` directory. You can run them via the GUT UI panel in the editor, or via CLI using the local Godot binary:
  ```bash
  /data/godot/bin/godot --headless -s addons/gut/gut_cmdln.gd
  ```
...
- **How to run**: Use the executable bash script in the root directory:
  ```bash
  ./run_integration_tests.sh
  ```
  This script uses a secondary config (`.gutconfig_integration.json`) and forces Godot to run with `--time-scale 2.0`, safely cutting the test execution time in half without breaking physics engine calculations.

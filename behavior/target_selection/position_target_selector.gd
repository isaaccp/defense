...
var condition_evaluators: Array[PositionConditionEvaluator] = []
...
func select_target(action: Action, actor: Actor, side_component: SideComponent) -> Target:
	...
	if def.sortable:
		var sorter = TargetSorterFactory.make_position_target_sorter(def.sort())
	...
	for target in targets:
		...
		return ConditionalTarget.make_position_conditional_target(target, condition_evaluators)
